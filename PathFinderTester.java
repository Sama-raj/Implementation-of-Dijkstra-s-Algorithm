...
class PathFinderTester {
	 ...
	 *
	 * @param args Two arguments which are input filename and "y/n" indicating
	...
	public static void main(String[] args) {
		...
		// -o <inputFilename> specifies the file that stores the shortest path results
		// (optional)
		if (options.has("o")) {
		...
		try {
			...
			if ((line = reader.readLine()) != null) {
				...
				if (tokens.length != 2) {
					System.err.println("There should be two numbers representing the number of rows and columns.");
				} else {
			...
			if ((line = reader.readLine()) != null) {
				...
				if (tokens.length < 2 || tokens.length % 2 != 0) {
					System.err.println("Origin coordinates should be in pairs.");
				} else {
			...
			if ((line = reader.readLine()) != null) {
				...
				if (tokens.length < 2 || tokens.length % 2 != 0) {
					System.err.println("Destination coordinates should be in pairs.");
				} else {
					for (int i = 0; i < tokens.length; i += 2) {
						int r = Integer.parseInt(tokens[i]);
						int c = Integer.parseInt(tokens[i + 1]);
			...
			while ((line = reader.readLine()) != null) {
				...
				if (tokens.length != 2) {
					System.err.println("Impassable coordinates should be in pairs");
				} else {
		...
		if (terrainFilename != null) {
			...
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					if (tokens.length != 3) {
						System.err.println("Terrain should be two coordinates and cost");
					} else {
						int cost = Integer.parseInt(tokens[2]);
						if (cost < 1) {
							System.err.println("Terrain cost must be 1 or more");
						} else {
		...
		if (waypointFilename != null) {
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					if (tokens.length != 2) {
						System.err.println("Waypoints should be two coordinates");
					} else {
		...
		if (path.size() == 0) {
			...
			outStream.println("Number of coordinates visited = " + (count));
